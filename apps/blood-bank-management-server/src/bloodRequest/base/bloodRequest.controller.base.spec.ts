...
const service = {
  ...
  bloodRequests: () => FIND_MANY_RESULT,
  bloodRequest: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("BloodRequest", () => {
  ...
  test("GET /bloodRequests/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/bloodRequests"}/${nonExistingId}`)
  ...
  test("GET /bloodRequests/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/bloodRequests"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /bloodRequests existing resource", async () => {
